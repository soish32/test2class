	...
	protected  void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(command.equals("boardlist")) { //글목록 보여주기
			...
			if(seqs==null||seqs.length==0) {
				jsForward("HkController.do?command=boardlist","최소하나이상 체크하세요",response);
			}else {
				boolean isS=dao.muldel(seqs);
				if(isS) {
					jsForward("HkController.do?command=boardlist","여러글삭제합니다", response);
				}else {

					jsForward("HkControllerdo?command=boardlist","여러글을삭제실패",response);
				}
